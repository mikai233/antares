...
import com.mikai233.config.luban.*
import com.mikai233.config.luban.query.*
...
class GameTablesTest {
    ...
    fun generatedLubanTablesLoadFromPublishedArtifacts() = runBlocking {
        ...
        assertEquals(com.mikai233.config.luban.gen.item.ItemType.Equipment, sword.type)
        ...
        assertEquals(
            ...
            snapshot.tbItem.all().count { row ->
                row.type == com.mikai233.config.luban.gen.item.ItemType.Equipment
            },
        )
        assertEquals(1, itemQueries.itemsByType[com.mikai233.config.luban.gen.item.ItemType.Equipment]?.size)
