...
import com.mikai233.shared.assets.*
...
class AssetTest {
    ...
    fun testAsset() {
        ...
        assert(assets1.aggregate<ItemAsset>(1) != null)
        val item2 = assets1.toBuilder().minus(item1).build().aggregate<ItemAsset>(1)!!
        assert(item2.num == 1L)
        ...
        val assets3 = AssetPackage.builder()
            ...
            .plus(resource2)
            .build()
        assert(assets3.aggregate<ResourceAsset>(2) == ResourceAsset(2, 100))
        assert(assets3.aggregate<ItemAsset>(1) == ItemAsset(1, 3))
    ...
    fun testDls() {
        ...
        assert(assets.aggregate<ItemAsset>(1) == ItemAsset(1, 1))
        assert(assets.aggregate<ItemAsset>(2) == ItemAsset(2, 100))
        assert(assets.aggregate<ResourceAsset>(1) == ResourceAsset(1, 1))
        assert(assets.aggregate<ResourceAsset>(2) == ResourceAsset(2, 200))
    }
